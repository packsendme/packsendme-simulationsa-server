package com.packsendme.microservice.simulationsa.service;
...
@Service
public class SimulationsaService {
	
	@Autowired
	private Configuration configuration;

	public ResponseEntity<?> findDistancesByCity(String origin, String destination) {
		Response<SimulationsaDto> responseObj = null;
		SimulationsaDto location = new SimulationsaDto();
		try {
			...
			if (response.getStatusCode() == HttpStatus.OK) {
				...
				responseObj = new Response<SimulationsaDto>(0,HttpExceptionPackSend.GOOGLEAPI_PLACE.getAction(), location);
			...
			e.printStackTrace();
			responseObj = new Response<SimulationsaDto>(0,HttpExceptionPackSend.FAIL_EXECUTION.getAction(), null);
